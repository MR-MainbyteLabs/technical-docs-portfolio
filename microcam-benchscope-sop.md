...
**Platform/Language:** Linux (Ubuntu 24.04 LTS / Debian 12) — Python 3.10+\
...
The application ships in two editions — choose one based on your hardware and preference:

| Edition | Entry Point | Best For |
| --- | --- | --- |
| FFmpeg | `modern/microcam_hyperlab_ffmpeg.py` | Simpler dependency model, maximum compatibility, easier troubleshooting |
| PyAV | `modern/microcam_hyperlab_pyav.py` | Cleaner architecture, more advanced recording path |
...
| --- | --- |
| OS | Ubuntu 24.04 LTS or Debian 12 (64-bit) recommended; Ubuntu 22.04 LTS also supported |
...
Add your user to the `video` group so the application can access the capture card without root:

```bash
sudo usermod -aG video $USER
```

> **You must log out and back in (or reboot) after this step for the group change to take effect.**
...
Choose the edition that matches your setup (see Section 1 for guidance):
...
Core dependencies installed by either requirements file:
...
| av | HDMI capture via PyAV backend (PyAV edition only) |
...
```bash
python3 -c "import cv2, PySide6, pytesseract; print('All imports OK')"
```

**PyAV edition:**

```bash
python3 -c "import cv2, PySide6, av, pytesseract; print('All imports OK')"
```

Expected output: `All imports OK`

If you see an import error, confirm the relevant system dependency from Section 3.1 is installed and that your virtual environment is activated.
...
Confirm 1920×1080 at 30fps (MJPG) is listed. If not, use the highest available resolution.

You can also verify the device is accessible to your user with:

```bash
ls -l /dev/video0
```

The device should be owned by the `video` group, and your user should be a member (confirmed in Section 3.1).

### 4\.2 Verify the Live Feed Before First Use

Test the capture card with ffplay before launching the application:

```bash
ffplay /dev/video0
```

Or explicitly with V4L2:

```bash
ffplay -f v4l2 /dev/video0
```

You should see a live feed. If the screen is black or blank, see Section 7.1 before proceeding.
...
### 4\.4 Complete Calibration Before Daily Use

Before daily use begins, complete the calibration procedure (Section 6) to set the pixel-to-mm ratio. Measurements will not be accurate until calibration is done.
...
- If using HDMI capture: confirm the feed is not showing a black border (adjust microscope HDMI output resolution if needed — target 1920×1080 MJPG at 30fps).
...
- Pixel-to-mm conversion is applied automatically using the calibrated value.
...
2. Confirm the reading matches the known dimension within ±0.05mm at your standard inspection zoom level. At lower magnification (wider field of view), ±0.1mm is acceptable.
...
| Device node correct? | Run `v4l2-ctl --list-devices` and confirm your device. |
| Resolution/FOURCC mismatch? | Set resolution to 1920×1080, format MJPG, 30fps. |
| Capture card already in use? | Close any other application using the device and relaunch. |
| HDMI source active? | Confirm the microscope HDMI output is active and the cable is seated. |

Test the raw feed outside the app to isolate the issue:
...
```
QT platform plugin error / could not initialize display
```

Set the platform explicitly before launching:
...
On Wayland systems, X11/XCB compatibility mode is required. You can make this permanent by adding `export QT_QPA_PLATFORM=xcb` to your `~/.bashrc`.
...
| Low contrast | Adjust lighting angle; reduce glare; enable CLAHE in-app if available |
| Non-English characters | Install the appropriate Tesseract language pack and update the language setting in-app |

Install additional Tesseract language packs (replace `deu` with your language code):
...
Full language code list: https://tesseract-ocr.github.io/tessdoc/Data-Files-in-different-versions.html

### 7\.4 Measurements Are Inaccurate

- Confirm calibration has been completed (Section 6) — the app uses a ratio of 1.0 (uncalibrated) until calibration is saved.
...
Check PyAV version (PyAV edition only):

```bash
python3 -c "import av; print(av.__version__)"
```

If PyAV throws an import error after a distro upgrade (ABI mismatch):

```bash
pip install --upgrade av
```

If the issue persists, switch to the FFmpeg edition (`requirements-ffmpeg.txt`) as a fallback.
...
- Adjust sensitivity in **Settings → Bridge Detection → Sensitivity** (reduce sensitivity to reduce false positives).
- All flagged regions require technician confirmation before being recorded as defects.

### 7\.7 Capture Card Not Detected

```bash
lsusb
```

MacroSilicon devices appear as `MacroSilicon USB Video`. If not listed, unplug and replug the card. Confirm you are using a USB 3.0 port.

Also confirm your user is in the `video` group:

```bash
groups
```

If `video` is not listed, run `sudo usermod -aG video $USER` and log out/in.
...
- [ ] Check for application updates:
  ...
  source venv/bin/activate
  pip install --upgrade -r requirements-ffmpeg.txt   # or requirements-pyav.txt
  ```
...
Recommended: daily rsync to a network share or external drive (replace `/mnt/backup` with your actual backup path):

```bash
rsync -av --progress $HOME/inspection_recordings/ /mnt/backup/inspection_recordings/
```

## Appendix A — Config Reference

microcam-benchscope does not use an external configuration file. Application settings (camera device, resolution, OCR options, recording path) are configured through the in-application UI and persisted by the application between sessions.

To identify your camera device node for selection in the app:
...
Recommended capture settings to select in-app:
...
| --- | --- | --- | --- |
| MacroSilicon MS2109 | `uvcvideo` | 1920×1080 @ 30fps | Plug-and-play; use PyAV or FFmpeg backend |
| MacroSilicon MS2130 | `uvcvideo` | 3840×2160 @ 30fps | Plug-and-play; use PyAV or FFmpeg backend |
| Generic UVC | `uvcvideo` | Varies | Use FFmpeg edition for maximum compatibility |
...
| Launch (FFmpeg edition) | `python3 modern/microcam_hyperlab_ffmpeg.py` |
| Launch (PyAV edition) | `python3 modern/microcam_hyperlab_pyav.py` |
| List capture devices | `v4l2-ctl --list-devices` |
| Check device formats | `v4l2-ctl --device=/dev/video0 --list-formats-ext` |
| Test raw feed | `ffplay /dev/video0` |
| Fix Qt platform error | `export QT_QPA_PLATFORM=xcb` then relaunch |
...
| Verify calibration | Measure a known feature; must read within ±0.05mm at standard zoom |
| Check for updates | `git pull origin main && pip install --upgrade -r requirements-ffmpeg.txt` |
| Backup recordings | `rsync -av --progress $HOME/inspection_recordings/ /mnt/backup/inspection_recordings/` |
