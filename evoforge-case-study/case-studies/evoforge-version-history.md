...
## v1 → v2: from simulation to spectacle
...
v2 added a presentation layer on top of the same simulation core:
...
- **A "documentary" auto\-camera** — `world.py` gained `pick_notable_agent()`,
  which picks whichever living agent is most interesting right now (oldest,
  deadliest, most prolific, etc.); `app.py` calls it on a timer and smoothly
  lerps the camera to the target, turning idle\-watching into something closer
  to a nature documentary.
- **Renderer** (`render.py`) was refactored from 563 to 411 lines — trails,
  captions, and the new event/flash data were added while dead code from the
  v1 renderer was removed in the same pass.
...
## v2 → v2.2: cutting scope, hardening what stayed
...
flavor text. The camera feature came back as **`X` — spotlight mode**, a simpler
rotate\-through\-criteria auto\-follow (kills → age → radius → children by fixed
time window). `pick_notable_agent()` was moved from `world.py` into `app.py`
(presentation logic no longer living in the simulation), and the camera now
snaps directly to the target position rather than lerping — simpler, and
consistent with the consolidation theme of this release.

The event pipeline in `world.py` was renamed and simplified —
