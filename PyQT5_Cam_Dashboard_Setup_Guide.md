...
| `numpy` | Array type for camera frame data |
...
> **RTSP URL format note:** The app constructs the full RTSP URL automatically from the
> IP address, username, and password you enter. It always connects on port **554** using
> the path `/cam/realmonitor?channel=1&subtype=0`, which is standard for Dahua and
> Amcrest-compatible cameras. If your camera uses a different port or RTSP path, you must
> edit the `RTSP_PATH` constant at the top of `camera_dashboard.py` before running the app.
> The tested URL format is:
...
> **Note:** The same RTSP port and path constraints from Phase 2 apply here. The URL is
> constructed automatically — only the IP, username, and password are entered.
...
| Camera connects but stream is black or errors immediately | Camera uses a non-standard RTSP port or path | Edit `RTSP_PATH` in `camera_dashboard.py` to match your camera's stream URL |
...
| App opens with no camera tiles after first run | Setup wizard was cancelled or no cameras were saved | Delete `camera_config.json` if it was created, rerun the app, and complete the wizard |
